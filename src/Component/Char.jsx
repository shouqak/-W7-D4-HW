  ...
  <div className="flex flex-row-reverse flex-wrap justify-center gap-6 p-5 ">
    {characters.length > 0 ? (
      characters.map((character) => (
        <div
          key={character.id}
          className="group relative max-w-sm w-full sm:w-72 bg-amber-100 border border-amber-200 rounded-xl shadow-md overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-2"
        >
          <div className="overflow-hidden h-64 bg-amber-100">
            <img
              src={character.url}
              alt={character.name}
